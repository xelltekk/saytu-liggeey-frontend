  ...
  <div class="space-y-4">
    ...
    <section class="pilotage-panel rounded-3xl border p-4 shadow-sm sm:p-5">
      <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <button type="button" class="agenda-primary-button" :disabled="loading" @click="openRdvPage()">
            <Plus class="h-4 w-4" />
            Rendez-vous
          </button>
        ...
        >
          <div class="mb-2 flex items-center justify-between gap-2">
            ...
            <div class="flex items-center gap-1">
              ...
              <button type="button" class="agenda-day-add" title="Ajouter un rendez-vous" @click="openRdvPage(day)">
                +
              </button>
            </div>
          </div>
...
function openRdvPage(day = null) {
  router.push({ name: 'agenda-rdv-create', query: day?.key ? { date: day.key } : {} })
}

function handleEventClick(event) {
  sessionStorage.setItem('saytu_agenda_event', JSON.stringify(event))
  router.push({
    name: 'agenda-event-detail',
    params: { id: String(event.id) },
    query: { month: agenda.value.month || agendaMonth.value },
  })
}
