    ...
    <template v-else-if="section === 'performance'">
      <div v-if="canManage" class="grid gap-4 xl:grid-cols-2">
        <form class="panel grid gap-2 sm:grid-cols-2" @submit.prevent="saveObjectif">
          ...
          <label class="field-label">Unite<input v-model="objectifForm.unite" class="input" placeholder="%, montant, clients..." required /></label>
...
function nom(e) { return e ? `${e.prenom || ''} ${e.nom || ''}`.trim() : '-' } function date(v) { return v ? new Date(v).toLocaleDateString('fr-FR') : '-' } function normalizeDate(v) { return v ? String(v).slice(0, 10) : null } function progression(o) { return Math.min(100, Math.round((Number(o.realise) / Math.max(1, Number(o.cible))) * 100)) } function money(v) { return new Intl.NumberFormat('fr-FR', { maximumFractionDigits: 0 }).format(Number(v || 0))  }
